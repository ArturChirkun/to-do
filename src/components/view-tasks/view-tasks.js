...
const ViewTasks = ({ tasks, deleteTask, completed, addToCompleted }) => {
  ...
  const listOfTasks = completed ?  tasks.filter((task) => task.completed === true) : tasks.filter((task) => task.completed === false);
  const header = completed ? 'Completed' : 'Incomplete';
  const noneTasks = listOfTasks.length ?  null  : "None tasks ";
  return (
    <div className="incomplete">
      <p className="head">{noneTasks}{noneTasks ? header.toLowerCase() : header }</p>
